...
| P6 | Phase 6: Tactical UI / UX (Radial Action Menu & Heads-Up Display) | 🟢 DONE (commit `b6a4b9f`; PO-approved deviation: radial overlay replaced by long-press Drop Report + Measure FAB; SOS/Ping removed from UI, dormant in VM; TacticalHudBar: WGS84-ITM-MGRS cycle coords, GPS accuracy+altitude, bearer + outbox count, battery% + GPS strategy) | app/ui/map, app/util/Coordinates.kt | modernization-execution-plan.md Phase 6 |
| D1 | Dynamic Report Types (per-event user-defined categories) | 🟢 IMPLEMENTED — awaiting rules deploy + device smoke test (`ReportTypeDefinition` + `FieldReport.typeId`, dual-write legacy `Type` int back-compat, `events/{eventId}/report_types` CRUD + lazy builtin seeding, deletion resolution dialog w/ per-event remembered preference + Event Overview control, 14-entry icon catalog (Material Symbols rounded-filled + FA star-of-david) with dynamic map markers, generalized counters/labels, Room schema v7 `typeId` column, rules §2.4 + manager report update/delete — emulator validation deferred) | core/model, core/network, core/database, app/ui/{map,management,reports}, firestore.rules | ../specs/dynamic-report-types.md |
